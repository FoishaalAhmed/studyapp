        ...
        <div class="row mt-4">
            <div class="col-12">
                <div class="card">
                    @include('alert')
                    <div class="card-body">
                        <h4 class="header-title">{{ __('Update App User Category') }}</h4>
                        ...
                        <form action="{{ route('admin.app-user-categories.update', $appUserCategory->id) }}" method="post">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <h4 class="box-title">{{ __(':x Section', ['x' => $appUserCategory->type]) }}</h4>
                                <div class="col-lg-12 mb-3">
                                    ...
                                    <select class="form-control" name="category_id" data-toggle="select2" data-width="100%" id="category" required="">
                                        @foreach ($categories as $item)
                                            <option value="{{ $item->id }}" {{ $item->id == $appUserCategory->category_id ? 'selected' : '' }} >{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-lg-6 mb-3">
                                    ...
                                    <input type="text" name="title" class="form-control" placeholder="{{ __('Title') }}" required="" value="{{ old('title', $appUserCategory->title) }}">
                                </div>
                                <div class="col-lg-6 mb-3">
                                    <label for="age" class="form-label">{{ __(':x Category', ['x' => $appUserCategory->type]) }}</label>
                                    <select class="form-control select2-multiple" name="categories[]" data-toggle="select2" data-width="100%" multiple="multiple" data-placeholder="{{ __('Select :x Categories', ['x' => $appUserCategory->type]) }}" id="user-category">
                                        @php
                                            $categoryIds = explode(',', $appUserCategory->categories);
                                        @endphp
...
@section('js')
    ...
    <script>
        'use strict';
        var csrf = "{{ csrf_token() }}";
        var type = "{{ $appUserCategory->type }}";
        var url = "{{ route('get.sub-category-by-category') }}";
    </script>
    <script src="{{ asset('public/assets/backend/js/custom/user-category-edit.js') }}"></script>
@endsection