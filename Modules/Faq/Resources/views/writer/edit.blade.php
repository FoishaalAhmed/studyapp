        ...
        <div class="row mt-4">
            <div class="col-12">
                <div class="card">
                    @include('alert')
                    <div class="card-body">
                        <h4 class="header-title">{{ __('Update Faq') }}</h4>
                        ...
                        <form action="{{ route('writer.faqs.update', $faq->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row mb-3">
                                <label for="question" class="col-4 col-xl-2 offset-lg-3 col-form-label text-end">{{ __('Question') }}</label>
                                <div class="col-8 col-xl-4">
                                    <input type="text" name="question" id="question" class="form-control" placeholder="{{ __('Question') }}" required="" value="{{ old('question', $faq->question) }}">
                                    @error('question')
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-8 col-xl-4">
                                    <textarea name="answer" id="answer" class="form-control" placeholder="{{ __('Answer') }}" required="" rows="5">{{ old('answer', $faq->answer) }}</textarea>
                                    @error('answer')
                                        <div class="invalid-feedback error">
                                            {{ $message }}
                                        </div>
