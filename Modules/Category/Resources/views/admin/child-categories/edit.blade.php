...
@section('title', __('Update Child Category'))
        ...
        <div class="row mt-4">
            <div class="col-12">
                <div class="card">
                    @include('alert')
                    <div class="card-body">
                        <h4 class="header-title">{{ __('Update Child Category') }}</h4>
                        <p class="text-muted font-13 mb-4 text-end mt-n4">
                            <a href="{{ route('admin.child-categories.index') }}" class="btn btn-outline-primary waves-effect waves-light"><i class="fe-list"></i> {{ __('All Child Category') }}</a>
                        </p>
                        <form action="{{ route('admin.child-categories.update', $childCategory->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-lg-9">
                                        ...
                                        <div class="col-8 col-xl-8">
                                            <select class="form-control" name="type" id="type" data-toggle="select2" data-width="100%" required="">
                                                @foreach ($types as $type)
                                                    <option value="{{ $type->id }}" {{ $type->id == old('type') || $childCategory->type == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('type')
                                        ...
                                        <div class="col-8 col-xl-8">
                                            <select class="form-control" name="sub_category_id" id="sub_category_id" data-toggle="select2" data-width="100%" required="">
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}" {{ $category->id == old('sub_category_id') || $childCategory->sub_category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-8 col-xl-8">
                                            <input type="text" name="name" id="name" class="form-control" placeholder="{{ __('Name') }}" required="" value="{{ old('name', $childCategory->name) }}">
                                            @error('name')
                                                <div class="invalid-feedback error">
                                                    {{ $message }}
                                                </div>
                                ...
                                <div class="col-lg-3">
                                    <div class="card card-border">
                                        <img class="card-img-top img-fluid" id="category-photo" src="{{ asset($childCategory->photo) }}" alt="{{ __('Category Image') }}">
                                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-lg-12 text-center">
                                    <a href="{{ route('admin.child-categories.index') }}" class="btn btn-outline-danger waves-effect waves-light"><i class="fe-delete"></i> {{ __('Cancel') }}</a>
                                    <button type="childmit" class="btn btn-outline-success waves-effect waves-light"><i class="fe-plus-circle"></i> {{ __('Submit') }}</button>
                                </div>
                            </div>
                        </form>
                    </div> <!-- end card body-->
