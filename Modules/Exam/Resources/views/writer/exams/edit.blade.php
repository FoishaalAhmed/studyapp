                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <p class="text-muted font-13 mb-4 text-end mt-n4">
                            <a href="{{ route('writer.exams.index') }}" class="btn btn-outline-primary waves-effect waves-light"><i class="fe-list"></i> {{ __('All Exam') }}</a>
                        </p>
                        
                        <form action="{{ route('writer.exams.update', $exam->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-lg-9">
                                    <div class="row mb-3">
                                        <div class="col-lg-4 mb-3">
                                            ...
                                            <select class="form-control" name="category_id" id="category_id" data-toggle="select2" data-width="100%" required="">
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}" {{ $category->id == old('category_id') || $category->id == $exam->category_id ? 'selected' : '' }}>{{ $category->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('category_id')
                                        ...
                                        <div class="col-lg-4 mb-3">
                                            ...
                                            <select class="form-control" name="exam_type" id="exam_type" data-toggle="select2" data-width="100%" required="">
                                                @foreach ($types as $type)
                                                    <option value="{{ $type->id }}" {{ $type->id == old('exam_type') || $type->id == $exam->exam_type ? 'selected' : '' }}>{{ $type->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('exam_type')
                                                <div class="invalid-feedback error">
                                        ...
                                        <div class="col-lg-4 mb-3" style="display: {{ $exam->exam_type == 1 || old('exam_type') == 1 ? 'none' : '' }}" id="subject-div">
                                            ...
                                            <select class="form-control" name="subject_id" id="subject_id" data-toggle="select2" data-width="100%">
                                                @foreach ($subjects as $subject)
                                                    <option value="{{ $subject->id }}" {{ $subject->id == old('subject_id') || $subject->id == $exam->subject_id ? 'selected' : '' }}>{{ $subject->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('subject_id')
                                                <div class="invalid-feedback error">
                                        ...
                                        <div class="col-lg-4 mb-3">
                                            ...
                                            <select class="form-control" name="type" id="type" data-toggle="select2" data-width="100%" required="">
                                                <option value="Free" {{ 'Free' == old('type') || $exam->type == 'Free' ? 'selected' : '' }}>{{ __('Free') }}</option>
                                                <option value="Premium" {{ 'Premium' == old('type') || $exam->type == 'Premium' ? 'selected' : '' }}>{{ __('Premium') }}</option>
                                            </select>
                                            @error('type')
                                                <div class="invalid-feedback error">
                                                    {{ $message }}
                                        ...
                                        <div class="col-lg-4 mb-3">
                                            ...
                                            <input type="text" name="note" id="note" class="form-control" placeholder="{{ __('Note') }}" value="{{ old('note', $exam->note) }}">
                                            @error('note')
                                                <div class="invalid-feedback error">
                                                    {{ $message }}
                                                </div>
                                ...
                                <div class="col-lg-3">
                                    <div class="card card-border">
                                        <img class="card-img-top img-fluid" id="exam-photo" src="{{ file_exists($exam->photo) ? asset($exam->photo) : asset('public/images/dummy/exam.webp') }}" alt="{{ __('Exam Image') }}">
                                        <div class="card-body">
