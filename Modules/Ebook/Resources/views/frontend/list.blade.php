        ...
        <div class="container">
            <div class="edu-sorting-area">
                <div class="sorting-left">
                    <h6 class="showing-text">{{ __('We found') }} <span>{{ $ebooks->total() }}</span>
                        {{ __('ebooks available for you') }}</h6>
                </div>
                    ...
                    <div class="layout-switcher">
                        ...
                        <ul class="switcher-btn">
                            @if (request()->is('ebooks/categories'))
                                <li>
                                    <a href="{{ route('ebooks.category', ['category' => request()->category, 'name' => request()->name, 'view' => 'grid']) }}" class=""><i class="icon-53"></i></a>
                                </li>
                            @elseif (request()->is('ebooks/subject'))
                                <li>
                                    <a href="{{ route('ebooks.subject', ['subject' => request()->subject, 'name' => request()->name, 'view' => 'grid']) }}" class=""><i class="icon-53"></i></a>
                                </li>
                            @elseif (request()->has('search'))
                                <li>
                                    <a href="{{ route('ebooks.search', ['search' => request()->search, 'view' => 'grid']) }}"
                                        class=""><i class="icon-53"></i></a>
                                </li>
                            @else
                                <li>
                                    <a href="{{ route('ebooks.grid', ['category' => request()->category, 'name' => request()->name]) }}"
                                        class=""><i class="icon-53"></i></a>
                                </li>
                            @endif
                                    ...
                                    <div class="inner">
                                        <div class="thumbnail">
                                            <a href="{{ route('ebooks.detail', ['title' => strtolower(str_replace([' ', '&', '_', '(', ')'], '-', $item->title)), 'ebook' => base64_encode($item->id)]) }}">
                                                <img src="{{ file_exists($item->photo) ? asset($item->photo) : asset('public/images/dummy/ebook.png')}}" alt="Course Meta" style="width: 320px;">
                                            </a>
                                        </div>
                                        <div class="content">
                                            <h4 class="title">
                                            ...
                                            <div class="read-more-btn">
                                                <a class="edu-btn btn-medium btn-border" href="{{ route('ebooks.detail', ['title' => strtolower(str_replace([' ', '&', '_', '(', ')'], '-', $item->title)), 'ebook' => base64_encode($item->id)]) }}">{{ __('Learn More') }} <i class="icon-4"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="content">
                                    @foreach ($categories as $category)
                                        <div class="edu-form-check">
                                            <a href="{{ route('ebooks.category', ['view' => 'list', 'category' => $category->id, 'name' => strtolower(str_replace([' ', '&', '_'], '-', $category->name))]) }}">{{ $category->name }}
                            ...
                            <div class="inner">
                                ...
                                <div class="content">
                                    @foreach ($subjects as $subject)
                                        <div class="edu-form-check">
                                            <a href="{{ route('ebooks.subject', ['view' => 'list', 'subject' => $subject->id, 'name' => strtolower(str_replace([' ', '&', '_'], '-', $subject->name))]) }}">{{ $subject->name }}
