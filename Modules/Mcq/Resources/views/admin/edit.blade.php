...
@section('title', __('Update Model Test'))
        ...
        <div class="row mt-4">
            <div class="col-12">
                <div class="card">
                    @include('alert')
                    <div class="card-body">
                        <h4 class="header-title">{{ __('Update Model Test') }}</h4>
                        <p class="text-muted font-13 mb-4 text-end mt-n4">
                            <a href="{{ route('admin.mcqs.index') }}" class="btn btn-outline-primary waves-effect waves-light"><i class="fe-list"></i> {{ __('All Model Test') }}</a>
                        </p>
                        
                        <form action="{{ route('admin.mcqs.update', $model->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-lg-9">
                                    <div class="row mb-3">
                                        <div class="col-lg-6 mb-3">
                                            ...
                                            <select class="form-control" name="child_category_id" id="child_category_id" data-toggle="select2" data-width="100%" required="">
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}" {{ $category->id == $model->child_category_id ? 'selected' : '' }}>{{ $category->name }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-lg-6 mb-3">
                                            ...
                                            <select class="form-control" name="subject_id" id="subject_id" data-toggle="select2" data-width="100%">
                                                ...
                                                @foreach ($subjects as $subject)
                                                    <option value="{{ $subject->id }}" {{ $subject->id == $model->subject_id ? 'selected' : '' }}>{{ $subject->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('subject_id')
                                                <div class="invalid-feedback error">
                                        ...
                                        <div class="col-lg-6 mb-3">
                                            ...
                                            <input type="text" name="year" id="year" class="form-control" placeholder="{{ __('Year') }}" required="" value="{{ old('year', $model->year) }}">
                                            @error('year')
                                                <div class="invalid-feedback error">
                                                    {{ $message }}
                                                </div>
                                        ...
                                        <div class="col-lg-6 mb-3">
                                            ...
                                            <select class="form-control" name="type" id="type" data-toggle="select2" data-width="100%" required="">
                                                <option value="Free" {{ 'Free' == old('type') || $model->type ? 'selected' : '' }}>{{ __('Free') }}</option>
                                                <option value="Premium" {{ 'Premium' == old('type') || $model->type ? 'selected' : '' }}>{{ __('Premium') }}</option>
                                            </select>
                                            @error('type')
                                                <div class="invalid-feedback error">
                                                    {{ $message }}
                                        ...
                                        <div class="col-lg-4 mb-3" id="price-div" style="display:{{  $model->type == 'Free' ? 'none' : '' }}">
                                            <label class="form-label">{{ __('Price') }}</label>
                                            <input type="text" name="price" id="price" class="form-control" placeholder="{{ __('Price') }}" value="{{ old('price', $model->price) }}">
                                            @error('price')
                                                <div class="invalid-feedback error">
                                                    {{ $message }}
                                                </div>
                                ...
                                <div class="col-lg-3">
                                    <div class="card card-border">
                                        <img class="card-img-top img-fluid" id="mcq-photo" src="{{ file_exists($model->photo) ? asset($model->photo) : asset('public/images/dummy/about.jpg') }}" alt="{{ __('Mcq Image') }}">
                                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-lg-12 text-center">
                                    <a href="{{ route('admin.mcqs.index') }}" class="btn btn-outline-danger waves-effect waves-light"><i class="fe-delete"></i> {{ __('Cancel') }}</a>
