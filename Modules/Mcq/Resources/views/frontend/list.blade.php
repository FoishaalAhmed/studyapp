        ...
        <div class="container">
            <div class="edu-sorting-area">
                <div class="sorting-left">
                    <h6 class="showing-text">{{ __('We found') }} <span>{{ $mcq->total() }}</span> {{ __('content available for you') }}</h6>
                </div>
                    ...
                    <div class="layout-switcher">
                        ...
                        <ul class="switcher-btn">
                            @if (request()->is('mcq/category'))
                                <li><a href="{{ route('mcq.category', ['category' => request()->category, 'name' => request()->name, 'view' => 'grid']) }}"><i class="icon-53"></i></a></li>
                            @elseif (request()->has('search'))
                                <li><a href="{{ route('mcq.search', ['search' => request()->search, 'view' => 'grid']) }}"><i class="icon-53"></i></a></li>
                            @else
                                <li><a href="{{ route('mcq.grid', ['category' => request()->category, 'name' => request()->name]) }}"><i class="icon-53"></i></a></li>
                            @endif
                            ...
                            <div class="inner">
                                <div class="thumbnail">
                                    <a href="{{ route('mcq.detail', ['title' => strtolower(str_replace([' ', '&', '_', '(', ')'], '-', $item->title)), 'mcq' => base64_encode($item->id)]) }}">
                                        <img src="{{ file_exists($item->photo) ? asset($item->photo) : asset('public/images/dummy/mcq.jpg')}}" alt="Course Meta" style="height: 200px; width: 200px;">
                                    </a>
                                </div>
                                <div class="content">
                                    <div class="course-price">{{ $item->type == 'Premium' ? '৳' . $item->price : __('Free') }}
                                    </div>
                                    <h6 class="title">
                                        ...
                                        <li class="search-bar">
                                            <form action="{{ route('mcq.search') }}">
                                                <div class="input-group">
                                                    <input type="text" class="form-control" placeholder="{{ __('MCQ Search') }}" name="search">
                                ...
                                <div class="content">
                                    @foreach ($categories as $category)
                                        <div class="edu-form-check">
                                            <a href="{{ route('mcq.category', ['view' => 'list', 'category' => $category->id, 'name' => strtolower(str_replace([' ', '&', '_'], '-', $category->name))]) }}">{{ $category->name }}
